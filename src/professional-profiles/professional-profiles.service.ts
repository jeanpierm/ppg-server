...
export class ProfessionalProfilesService {
  ...
  async generateProfile(
    ...
    this.logger.log(`Professional profile generated by user ${user.userId}`, createdProfile);
  ...
  async findActiveProfileOfUserById(
    ...
    throw new ForbiddenException('This profile is not generated by the user who requested it');
  }

  async removeProfileOfUser(user: UserDocument, ppId: string): Promise<void> {
    const profile = await this.proProfileModel.findOne({ ppId, status: EntityStatus.Active });
    if (user._id.equals(profile.owner)) {
